...
const GlowingEffect = memo(
  ({
    ...
    const tweenFrameRef = useRef<number>(0);

    const animateAngle = useCallback(
      (element: HTMLDivElement, from: number, to: number) => {
        if (movementDuration <= 0 || from === to) {
          element.style.setProperty("--start", String(to));
          return;
        }

        if (tweenFrameRef.current) {
    ...
    const handleMove = useCallback(
      (e?: MouseEvent | { x: number; y: number }) => {
        ...
        interactionFrameRef.current = requestAnimationFrame(() => {
          ...
          animateAngle(element, currentAngle, newAngle);
        });
      },
